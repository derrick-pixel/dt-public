...
export async function loadAppData() {
  const [competitors, market, pricing, whitespace, designAudit, culture] = await Promise.all([
    ...
    loadJSON('data/design-audit.json').catch(() => null),
    loadJSON('data/culture-analytics.json').catch(() => null),
  ]);
  const data = { competitors, market, pricing, whitespace, designAudit, culture };
  window.AppData = data;
  return data;
}
