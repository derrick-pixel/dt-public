...
export async function getMagicLinkVerify(
  ...
  // Decide redirect: incomplete onboarding → /onboard.html, else /couple.html.
  ...
  const onboardingComplete =
    ...
    !!profile?.nric_consented_at &&
    (charityCount?.c ?? 0) > 0;
  const location = onboardingComplete ? '/couple.html' : '/onboard.html';
...
function htmlError(message: string): Response {
  const html =
    ...
    `<p><a href="/couple.html">Return to the wedding dashboard</a> to request a new sign-in link.</p>` +
    `</body></html>`;
