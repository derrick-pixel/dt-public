...
(function init() {
  const faction = sessionStorage.getItem('elixcraft_faction') || 'protoss';
  const badge   = sessionStorage.getItem('elixcraft_faction_badge') || 'PROTOSS · ENGINEERING';
...
function loadPlayerIntoCommandCard(state) {
  ...
  if (classEl) classEl.textContent = `${p.role?.title || 'Unit'} · ${(p.faction || 'PROTOSS').toUpperCase()}`;
...
// ─── Unit Selection → updates Command Card ──────────────────────────────────
...
function selectUnit(el, initials, name, role, level, xpPct, str, int, cha, dex, statusType) {
  ...
  document.querySelector('.portrait-class').textContent    = `${role} · PROTOSS`;
  ...
  // Update XP bar in command card if present
  const xpRow = document.querySelector('.portrait-xp');
  if (xpRow) xpRow.style.width = xpPct + '%';

  // Toast
  const statusMap = { active:'● ACTIVE', quest:'⚡ ON QUEST', training:'📚 TRAINING', ghost:'RECRUITING' };
    ...
    <div style="display:flex;flex-direction:column;gap:12px;">
      ...
      <div style="display:flex;gap:10px;margin-top:4px;">
        ...
        <button onclick="logQuestProgress('${name}');closeModal()" style="flex:1;font-family:'Orbitron',monospace;font-size:0.6rem;letter-spacing:0.15em;padding:10px;background:${color};color:#000;border:none;border-radius:2px;cursor:pointer;font-weight:700">
          ${status === 'available' ? '▶ START QUEST' : '+ LOG PROGRESS'}
        </button>` : ''}
        <button onclick="closeModal()" style="flex:1;font-family:'Orbitron',monospace;font-size:0.6rem;letter-spacing:0.15em;padding:10px;background:transparent;color:#5a7a9a;border:1px solid #1a3050;border-radius:2px;cursor:pointer">
          CLOSE
        </button>
...
function logQuestProgress(name) {
  // Bump matching quest progress in save state
  if (window.GameState && GameState.exists()) {
    const state = GameState.get();
...
const MODAL_CONTENT = {
  'skill-modal': {
    title: 'ADD POWER-UP — SKILL TRAINING',
    body: `
      <p style="font-size:0.78rem;color:#a0b8c8;line-height:1.5;margin-bottom:16px">Select a skill to add to your unit's power-up loadout. Completing training quests unlocks higher tiers.</p>
      <div style="display:flex;flex-direction:column;gap:10px;">
        ${[
          ...
          <div onclick="this.style.borderColor='${color}';this.style.background='rgba(255,255,255,0.03)'" style="display:flex;align-items:center;gap:12px;padding:10px 14px;background:#0a1628;border:1px solid #1a3050;border-radius:3px;cursor:pointer;transition:all 0.15s;">
            ...
            <button onclick="event.stopPropagation();addSkillToast('${name}')" style="font-family:'Orbitron',monospace;font-size:0.55rem;padding:7px 14px;background:${color};color:#000;border:none;border-radius:2px;cursor:pointer;letter-spacing:0.1em;font-weight:700">EQUIP</button>
          </div>
        `).join('')}
      </div>
    `,
  ...
  'benefit-modal': {
    title: 'UPGRADE ARMOR — BENEFITS',
    body: `
      <p style="font-size:0.78rem;color:#a0b8c8;line-height:1.5;margin-bottom:4px">Upgrade your unit's benefits package. All tiers comply with MOM Employment Act, CPF Board regulations, and SkillsFuture SG frameworks.</p>
      ...
      <div style="display:flex;flex-direction:column;gap:8px;">
        ${[
          ...
          ['🏦','CPF Voluntary Top-Up (VC)',      'VC-OA / VC-SA',     'Employer voluntary CPF top-up above mandatory 17%. Grows unit\'s retirement fund', '+10 XP'],
          ...
          ['📈','Equity / Share Scheme (RSU)',   'New grant',         '4-year vest, 1-year cliff. Performance-accelerated schedule for senior units',      '+15 STR'],
  ...
  'recruit-modal': {
    title: 'RECRUIT UNIT — OPEN POSITIONS',
    body: `
      <p style="font-size:0.78rem;color:#a0b8c8;line-height:1.5;margin-bottom:16px">Add a new unit to your roster. Source: SkillsFuture SG + MyCareersFuture 2025 salary benchmarks.</p>
      <div style="display:flex;flex-direction:column;gap:10px;">
        ${[
          ['⚡','Principal Engineer',   'PROTOSS','SGD $13,000–$18,000/mo','Supply: +1 · ENG track'],
          ['⚙️','Senior Data Scientist','PROTOSS','SGD $9,000–$12,000/mo', 'Supply: +1 · Data/AI track'],
          ['🏗️','Operations Manager',   'TERRAN', 'SGD $5,500–$8,500/mo',  'Supply: +1 · OPS track'],
          ['🦾','Business Dev Manager', 'ZERG',   'SGD $8,000–$13,000/mo', 'Supply: +1 · Sales track'],
          ['🤝','HR Business Partner',  'ZERG',   'SGD $6,000–$10,000/mo', 'Supply: +1 · People track'],
        ].map(([icon, name, faction, salary, supply]) => `
          <div style="display:flex;align-items:center;gap:12px;padding:10px 14px;background:#0a1628;border:1px solid #1a3050;border-radius:3px;">
            ...
            <div style="flex:1">
              <div style="font-size:0.82rem;color:#fff;font-weight:600">${name}</div>
              <div style="font-size:0.62rem;color:#5a7a9a;margin-top:2px">${faction} · ${supply}</div>
...
function addSkillToast(name)      { showToast(`⚡ Power-Up equipped: ${name}`, '#ffd700'); closeModal(); }
function recruitToast(name)       { showToast(`✓ Recruiting started: ${name}`, '#00e676'); closeModal(); }
function upgradeBenefitToast(name){ showToast(`🛡️ Armor upgraded: ${name}`, '#4a9eff');   closeModal(); }
...
function renderIndustryTab() {
  ...
  const factionLabels = { protoss: '⚡ PROTOSS', terran: '🏗️ TERRAN', zerg: '🦾 ZERG', multi: '◈ ALL' };
