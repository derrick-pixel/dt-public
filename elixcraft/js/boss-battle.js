// ELIXCRAFT — Boss Battle: Performance Review Engine
...
const char = (() => {
  try { return JSON.parse(sessionStorage.getItem('elixcraft_character')); } catch { return null; }
})();
...
const PLAYER_MOVES = [
  ...
  },
  {
    ...
    effect: (dmg) => `Boss scrutiny reduced by ${dmg}`,
    outcomeShift: 1,
...
const BOSS_MOVES = [
  ...
  { id: 'positive_signal',name: 'Positive Recognition', effect: 'Boss is impressed', shift: 1, hp_dmg: 0, morale_dmg: -10, msg: ['That launch was genuinely impressive.', 'The team feedback on you has been excellent.'] },
...
function playerMove(moveId) {
  ...
  logEntry(rollClass, `> YOU: ${move.name.split(' ').slice(1).join(' ')} — Roll ${roll}${crit?' 🎯 CRITICAL!':miss?' 💀 MISS':''}`);
  ...
  updateBars();
  updateOutcomeTrack();
  renderActions(true); // disable until boss acts
...
function bossMove() {
  ...
  logEntry('log-boss', `▶ BOSS: "${pickRandom(move.msg)}"`);
  ...
  const outcomes = [
    ...
    },
    {
      ...
      rewards: [
        ...
        { k: 'SALARY', v: 'No increase' }, { k: 'MORALE', v: '-5' }
      ],
      color: '#a0a0a0',
    },
    {
      ...
      rewards: [
        ...
        { k: 'MERIT RAISE', v: '+3% salary' }, { k: 'MORALE', v: '+5' }
      ],
      color: '#4a9eff',
    },
    {
      ...
      rewards: [
        ...
        { k: 'MERIT RAISE', v: '+6% salary' }, { k: 'BONUS', v: '1 month' }, { k: 'MORALE', v: '+15' }
      ],
      color: '#ffd700',
    },
    {
