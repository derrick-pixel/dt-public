// ELIXCRAFT — Career Fog of War Map (contextualised per role)
...
const ROLE_DESC = {
  ...
  se6:  'Unicorn-tier. Shapes industry direction. Speaks at conferences, publishes research, and defines what "good engineering" means across the organisation.',
  // PROTOSS — Data/AI
...
const LAYOUT = {
  protoss: [
    ...
    { track:'cybersec',   role:'cs2', col:1, row:5.5, icon:'🗡️'  },
...
function loadPlayer() {
  let level = 1, faction = 'protoss', currentRole = null, playerName = 'Commander';
  if (window.GameState && GameState.exists()) {
    ...
    const char = (() => { try { return JSON.parse(sessionStorage.getItem('elixcraft_character')); } catch { return null; } })();
    if (char) {
      faction     = char.faction      || 'protoss';
      currentRole = char.role?.id     || null;
      playerName  = char.name         || 'Commander';
      if (window.GameState && char.skills) {
        const xp = char.skills.reduce((a, s) => a + (s.xp || 0), 0);
        level = GameState.xpToLevel(xp);
      }
    } else {
      faction = sessionStorage.getItem('elixcraft_faction') || 'protoss';
    }
  }
...
function init() {
  // Activate the player's faction tab
...
function renderMap(faction) {
  ...
  layout.forEach(item => {
    ...
    // If we're viewing our own faction, use PLAYER context; otherwise neutral
...
function showDetail(role, item, state, factionColor, faction) {
  ...
  panel.innerHTML = `
    ...
    <div class="cdp-unlock-row" style="font-size:0.7rem;color:#a0b8c8;line-height:1.5">
      ${state === 'active'
        ? `✅ This is your active role (${PLAYER.playerName}). Complete quests and earn XP to advance. ${xpLabel}.`
        : state === 'unlocked'
        ? `✅ Level requirement met (LVL ${role.level[0]}+). ${roleSkills.length ? 'Equip required skills via POWER-UPS to maximise your rating.' : ''}`
        : state === 'locked'
        ? `🔒 Reach Level ${role.level[0]} and complete ${roleSkills.slice(0,2).map(s=>s.name).join(' + ')||'prerequisite training'} to unlock.`
        : `🌫️ This path is hidden. Keep levelling up to reveal what lies ahead.`
      }
    </div>

    <div style="margin-top:14px;display:flex;flex-direction:column;gap:8px">
      ${state === 'active'
        ? `<button class="cdp-btn" onclick="window.location='boss-battle.html'">⚔️ START PERFORMANCE REVIEW</button>`
        : state === 'unlocked'
      ...
      ${state !== 'fog' && state !== 'active'
        ? `<button class="cdp-btn" style="background:transparent;border:1px solid #2a4060;color:#5a7a9a;font-size:0.52rem"
             onclick="window.location='boss-battle.html'">⚔️ RUN PERFORMANCE REVIEW</button>`
        : ''
      }
    </div>
  `;
...
function setTargetRole(roleId, roleTitle) {
  sessionStorage.setItem('elixcraft_target_role', roleId);
  sessionStorage.setItem('elixcraft_target_role_title', roleTitle);
  ...
  showLevelUp('TARGET ROLE SET', `${roleTitle} — Complete quests to unlock this path`);
}
...
function clearDetailPanel() {
  document.getElementById('cdp-body').innerHTML = `
    ...
    <div class="cdp-tip">🗺️ Nodes behind the fog of war represent paths not yet visible. Level up and complete quests to reveal them.</div>
  `;
}
