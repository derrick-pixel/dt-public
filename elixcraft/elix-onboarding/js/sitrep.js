// ELIX ONBOARDING — Analytics Dashboard Logic
...
function renderNoSave() {
  document.querySelector('.sitrep-grid').innerHTML = `
    <div style="grid-column:1/-1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:20px;padding:80px 20px;text-align:center">
      ...
      <div style="font-size:0.8rem;color:var(--text-dim);max-width:360px;line-height:1.6">Complete the onboarding wizard first to begin generating analytics data.</div>
...
function renderProfile(state) {
  ...
  // Avatar initials
  const initials = (p.name || 'TM').split(' ').map(w => w[0]).join('').slice(0, 2).toUpperCase();
  document.getElementById('sr-avatar').textContent = initials;

  // Department colour for avatar
  ...
  document.getElementById('sr-name').textContent = p.name || 'Team Member';
  const fLabel = { protoss: '⚡ TECHNOLOGY · Engineering', terran: '🏗️ OPERATIONS · Operations', zerg: '🦾 GROWTH · Sales & Growth' };
  document.getElementById('sr-role').textContent = `${p.role?.title || 'Team Member'} · ${fLabel[p.faction] || 'Unknown Department'}`;
...
function renderResources(state) {
  ...
  const items = [
    { key: 'minerals', icon: '💎', name: 'BUDGET (SGD)',     val: (r.minerals || 0).toLocaleString(), pct: Math.min(100, (r.minerals || 0) / 2000), cls: 'res-minerals' },
    { key: 'gas',      icon: '🎯', name: 'TALENT',           val: r.gas || 0,                          pct: Math.min(100, (r.gas || 0) * 2),         cls: 'res-gas'      },
    { key: 'supply',   icon: '👥', name: 'HEADCOUNT',        val: r.supply || 0,                       pct: Math.min(100, (r.supply || 0) * 10),     cls: 'res-supply'   },
  ];
...
function renderBattles(state) {
  ...
  if (!battles.length) {
    el.innerHTML = '<div class="sr-no-data">No reviews recorded yet.<br>Complete the Annual Performance Review to begin.</div>';
    return;
  }

  const outcomeIcon = { win: '🏆', loss: '⚠️', draw: '📋' };
  ...
  el.innerHTML = battles.slice(0, 6).map(b => `
    <div class="sr-battle-item">
      <span class="sr-battle-icon">${outcomeIcon[b.outcome] || '📋'}</span>
...
function renderSectors(state) {
  ...
  el.innerHTML = ALL_SECTORS.map(s => {
    ...
    return `
      <div class="sr-sector-pip ${deployed ? 'deployed' : ''}" title="${s.name}${deployed ? ' — Active' : ''}">
...
function renderRoster(state) {
  ...
  const statusLabel = { active: '● ACTIVE', quest: '⚡ OBJECTIVE', training: '📚 TRAINING', ghost: '○ INACTIVE' };
