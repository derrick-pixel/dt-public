// ELIX ONBOARDING — Onboarding Wizard Logic
...
const FACTION_META = {
  protoss: { icon: '⚡', label: 'TECHNOLOGY', dept: 'Engineering & Technology',  lore: 'ICT · Data/AI · Cloud · Cybersecurity · Engineering Management', color: '#ffd700' },
  terran:  { icon: '🏗️', label: 'OPERATIONS',  dept: 'Operations & Finance',       lore: 'Operations · Finance · Procurement · Project Management',         color: '#4a9eff' },
  zerg:    { icon: '🦾', label: 'GROWTH',    dept: 'Sales, Marketing & People',  lore: 'Sales · Digital Marketing · HR & People · Product Management',    color: '#b44fff' },
};
...
function populateRoles() {
  ...
  // Department identity banner above tracks
  const banner = `
      ...
      ${track.roles.map(role => `
        <div class="role-option" data-faction="${character.faction}" data-track="${track.id}" data-role="${role.id}"
          ...
          <div class="ro-info">
            ...
            <div class="ro-meta">LVL ${role.level[0]}–${role.level[1]} · Headcount: ${role.supply_cost}</div>
...
function populateSummary() {
  ...
  const fLabel = { protoss: '⚡ TECHNOLOGY · Engineering', terran: '🏗️ OPERATIONS · Operations', zerg: '🦾 GROWTH · Sales & Growth' };
  const container = document.getElementById('summary-card');
  container.innerHTML = `
    <div>
      <div class="summary-section-title">TEAM MEMBER PROFILE</div>
      <div class="summary-row"><span class="summary-label">DEPARTMENT</span><span class="summary-value">${fLabel[character.faction] || '—'}</span></div>
      ...
      <div class="summary-row"><span class="summary-label">BENEFITS</span><span class="summary-value">${pkg?.name || '—'} (${pkg?.tier || '—'})</span></div>
    ...
    <div>
      ...
      <div style="margin-top:20px;padding:12px;background:var(--bg-deep);border:1px solid var(--border);border-radius:3px;font-size:0.7rem;color:var(--text-dim);line-height:1.6">
        ...
        Headcount cost: <strong>1 position</strong>
      </div>
    </div>
  `;
}

// ─── Auto-skip Step 1 if department already chosen on index.html ───────────────
// select.js saves elixonboarding_faction to sessionStorage before redirecting here.
// If it exists, pre-select that department silently and jump straight to Step 2.

(function autoAdvanceFaction() {
  const saved = sessionStorage.getItem('elixonboarding_faction');
  if (!saved) return;
  ...
  showStep(2);                          // skip department step entirely
})();

// ─── Complete Onboarding ─────────────────────────────────────────────────────

function deployUnit() {
  sessionStorage.setItem('elixonboarding_character', JSON.stringify(character));
  sessionStorage.setItem('elixonboarding_faction',   character.faction);
  sessionStorage.setItem('elixonboarding_faction_badge',
    `${character.faction === 'protoss' ? 'TECHNOLOGY' : character.faction === 'terran' ? 'OPERATIONS' : 'GROWTH'} · ${character.role?.title || 'TEAM MEMBER'}`);
