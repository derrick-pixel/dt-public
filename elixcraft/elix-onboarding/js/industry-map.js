// ELIX ONBOARDING — Industry Sector Map
...
const PLAYER_FACTION = sessionStorage.getItem('elixonboarding_faction') || 'protoss';
...
(function init() {
  ...
  const badgeMap = {
    protoss: 'TECHNOLOGY · ENGINEERING',
    terran:  'OPERATIONS · OPERATIONS',
    zerg:    'GROWTH · SALES & PEOPLE',
  };
  const badge = document.getElementById('im-faction-badge');
  if (badge) badge.textContent = badgeMap[PLAYER_FACTION] || 'TECHNOLOGY · ENGINEERING';
...
function buildSectorCard(sector) {
  ...
  const statusLabel = { deployed: '● ACTIVE', available: '◈ AVAILABLE', restricted: '○ RESTRICTED' }[sector.status] || 'AVAILABLE';
...
function showSectorDetail(sector) {
  ...
  const affinityClass = `is-affinity-${sector.faction}`;
  const affinityLabel = {
    protoss: '⚡ TECHNOLOGY ALIGNMENT',
    terran:  '🏗️ OPERATIONS ALIGNMENT',
    zerg:    '🦾 GROWTH ALIGNMENT',
    multi:   '◈ ALL-DEPARTMENT SECTOR',
  }[sector.faction];
  ...
  panel.innerHTML = `
    ...
    <div class="imd-bonus-box" style="border-color:rgba(90,122,154,0.3);color:#5a7a9a;background:rgba(255,255,255,0.02)">
      ⚠️ Entering this sector without ${sector.faction === 'protoss' ? 'TECHNOLOGY' : sector.faction === 'terran' ? 'OPERATIONS' : 'GROWTH'} alignment costs +25% budget. Recruit aligned team members first.
    </div>`}
    ...
    <div class="imd-section-title">REQUIRED SKILLS</div>
...
function deployToSector(id, name) {
  sessionStorage.setItem('elixonboarding_target_sector', id);
  sessionStorage.setItem('elixonboarding_target_sector_name', name);
  ...
  showToast(`🚀 Sector assigned: ${name} — team members now routing to this sector`, '#00e676');

  // Re-render panel with active state
