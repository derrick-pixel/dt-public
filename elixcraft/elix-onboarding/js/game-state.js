// ELIX ONBOARDING — State Manager (localStorage persistence)
// All state lives in localStorage under key 'elixonboarding_save'

// ─── Role-Based Onboarding Objective Templates ───────────────────────────────────
const ROLE_QUEST_TEMPLATES = {
  protoss: [
    { id: 'ob1', name: 'Complete Pre-Start Briefing',               type: 'ONBOARDING', status: 'active',    progress: 0, deadline: 'Day 1',       reward: '+185 XP, Onboarding Clearance' },
    { id: 'ob2', name: 'Set Up Dev Environment & Access',         type: 'ONBOARDING', status: 'available', progress: 0, deadline: 'Day 3',        reward: '+30 XP, +5 INT' },
    { id: 'ob3', name: 'Complete Codebase Architecture Review',   type: 'TRAINING',   status: 'available', progress: 0, deadline: 'Day 7',        reward: '+40 XP, Career Pathway unlock' },
...
const GameState = (() => {
  ...
  const DEFAULT = {
    ...
    player: {
      ...
      faction: 'protoss',
      track:   null,
      role:    { id: null, title: 'Team Member', salary: [0, 0] },
      level:   1,
      xp:      0,
      xpToNext: 100,
    ...
    quests: [
      { id: 'q1', name: 'Complete Onboarding Module',   type: 'TRAINING',  status: 'active',    progress: 0,  deadline: 'End of Q2 2026', reward: '+50 XP, +$200 budget' },
      { id: 'q2', name: 'Deliver Q2 Sprint Goals',      type: 'MILESTONE', status: 'available', progress: 0,  deadline: 'Jun 2026',        reward: '+3% salary increment' },
      { id: 'q3', name: 'Mentor a Junior Team Member',  type: 'LEADERSHIP',status: 'available', progress: 0,  deadline: 'Ongoing',          reward: '+20 XP, +5 CHA' },
      { id: 'q4', name: 'SkillsFuture Certification',   type: 'TRAINING',  status: 'available', progress: 0,  deadline: 'Dec 2026',         reward: '+30 XP, +8 INT' },
      { id: 'q5', name: 'Annual Performance Review',    type: 'BOSS',      status: 'available', progress: 0,  deadline: 'Q4 2026',          reward: 'Promotion + +15% raise' },
    ],
  ...
  function create(characterData) {
    ...
    if (characterData) {
      state.player.name    = characterData.name    || 'Team Member';
      ...
      // Inject role-based onboarding objective chain
  ...
  function reset() {
    ...
    // Also clear session
    sessionStorage.removeItem('elixonboarding_character');
    sessionStorage.removeItem('elixonboarding_faction');
    sessionStorage.removeItem('elixonboarding_faction_badge');
  }
  ...
  function addXP(amount) {
    ...
    state.player.level = xpToLevel(state.player.xp);
    state.resources.minerals += Math.floor(amount * 10); // XP → budget
    save(state);
    return state;
  }
